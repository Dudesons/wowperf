...
def a_run_with_duplicate_names() -> Run:
    ...
    return Run(
        ...
        count_reached=100, count_required=100, npc_counts=(),
        players=(
            Player(actor_id=0, name="Alpha", class_name="Mage", spec="Arcane",
                   item_level=318),
            Player(actor_id=1, name="Bravo", class_name="Shaman", spec="Elemental",
                   item_level=311),
            Player(actor_id=2, name="Charlie", class_name="Priest", spec="Holy",
                   item_level=311),
            Player(actor_id=3, name="Delta", class_name="Rogue", spec="Outlaw",
                   item_level=311),
            Player(actor_id=5, name="Alpha", class_name="DeathKnight", spec="Blood",
                   item_level=311),
        ),
        pulls=pulls,
    )
...
def test_low_activity_players_sharing_a_name_get_distinct_finding_ids() -> None:
    # Bravo, Charlie and Delta cast nothing either, so they also come back as low
    # activity; what matters here is that the two Alphas do not collide.
    run = a_run_with_duplicate_names()
    findings = analyse_players(run, (cast(0, 1_000), cast(5, 2_000)), (), (), ())
    activity_ids = {f.id for f in findings if f.id.startswith("players.activity.")}
    assert {"players.activity.Alpha.0", "players.activity.Alpha.5"} <= activity_ids


def test_damage_outlier_players_sharing_a_name_are_distinguished_in_the_title() -> None:
    run = a_run_with_duplicate_names()
    damage = (
