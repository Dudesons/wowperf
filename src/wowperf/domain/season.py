...
class CooldownAbility(Frozen):
    ...
    Shared by the defensive and offensive lists, which differ in what they mean
...
class OffensiveCooldowns(Frozen):
    ...
    dying is a loss; an offensive cooldown unpressed on a trivial pack is
    ...
    def for_spec(self, class_name: str, spec: str) -> tuple[CooldownAbility, ...]:
        """The known offensive cooldowns for a class/spec, or `()` if none are listed."""
        wanted = f"{class_name}/{spec}"
        for key, abilities in self.entries:
            if key == wanted:
                return abilities
