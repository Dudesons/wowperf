...
flowchart TB
    ...
    subgraph facts["PYTHON — produces facts"]
        ...
        DOM --> AN["16 analysers<br/>deaths, interrupts,<br/>defensives, tempo..."]
...
Four skills ship in `.claude/skills/`, 2,114 lines in total. Three are surfaced to the harness and
...
flowchart LR
    ...
    W["<b>wcl-api</b><br/>1,180 lines<br/><i>the verified schema</i>"]
...
**`wcl-api`** is the largest artefact in the repository at 1,180 lines, and it exists because of a
specific scar:
...
`docs/plans/` is the fossil record — **55 documents, 17 matched design/plan pairs sharing a date**.
...
The scale, for calibration: **580 commits across 14 days, 21 merged pull requests**, and a test
suite of **2,040 tests over 39,000 lines — roughly twice the line count of the source it covers.**
...
```mermaid
flowchart TB
    CLI["<b>cli.py</b> — 1,728 lines · Typer app · 4 commands<br/><i>the only place ports meet implementations</i>"]

    subgraph domain["DOMAIN — 100 files, 20,071 lines, zero I/O"]
        direction TB
        MODEL["<b>model · events · encounter · findings</b><br/>frozen pydantic value objects"]
        ANA["<b>analysis/</b> — 16 modules"]
    ...
    subgraph driven["DRIVEN ADAPTERS — all the I/O lives here"]
        ...
        REND["<b>render/</b> — Jinja2, 14 templates"]
        WCL --> CACHE
    end
...
```
src/wowperf/
├── cli.py                 driving adapter: wires ports, 4 commands (1,728 lines)
...
│   ├── analysis/          16 modules — the findings
...
└── adapters/
    ...
    └── render/            html.py + icons.py + 14 Jinja templates
```
...
| `progression` | `LoadedProgression` | one analyser | progression JSON only |
...
| `progression_service.py` | where a night's attempts sat, and whether the night moved |
...
Every command prints a quota sentence and a per-operation breakdown to stderr. Budget is 3,600
points an hour; a full run fetch costs roughly six.
...
| `uv run pytest` | offline suite — 2,022 tests, no credentials, ~20 seconds |
...
| Area | Files | Tests collected |
| --- | --- | --- |
| `tests/domain/report/` | 24 | 485 |
| `tests/domain/comparison/` | 20 | 455 |
| `tests/domain/analysis/` | 17 | 279 |
| `tests/adapters/wcl/` | 21 | 256 |
| `tests/adapters/render/` | 6 | 214 |
| `tests/` (CLI, skills, urls, ports) | 5 | 168 |
| `tests/domain/` | 11 | 109 |
| `tests/adapters/config` + `cache` | 3 | 56 |
| `tests/e2e/` | 9 | 18 |
| **Total** | **116** | **2,040** |
